...
class MLP(nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, num_layers,
                 std_w=params("std_w"), dropout=ratio("dropout"), beta = 0):
        ...
        self.fc_std = nn.Linear(hidden_channels, hidden_channels)
        self.fc_q_x_z = nn.Linear(hidden_channels, in_channels)
        self.beta = beta
        self.std_w = std_w
    ...
    def reset_parameters(self):
        ...
        self.fc_std.reset_parameters()
        self.fc_q_x_z.reset_parameters()
    ...
    def forward(self, x, edge_index=None):
        ...
        mu = self.fc_mu(x)
        std = F.softplus(self.fc_std(x) - self.std_w, beta=ratio("softplus_beta"))
        z = self.reparameterize(mu, std)
        logits = self.lins[-1](z)
        x_recon = self.fc_q_x_z(z)
        return logits, z, mu, std, x_recon, self.beta, self.std_w
...
class SGC(nn.Module):
    def __init__(self, in_channels, out_channels, hops=params("hops")):
...
class GCNIB(nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, num_layers=params("num_layers"), dropout=ratio("dropout"), \
                 std_w = params("std_w"), use_bn = True, save_mem=True):
        super(GCNIB, self).__init__()
        ...
        self.fc_mu = nn.Linear(hidden_channels, hidden_channels)
        self.fc_std = nn.Linear(hidden_channels, hidden_channels)
        self.fc_x_a_z = GCNConv(hidden_channels, in_channels)
        self.q_x_z_mu = nn.Linear(hidden_channels, in_channels)
        self.q_x_z_logvar = nn.Linear(hidden_channels, in_channels)
    ...
    def reset_parameters(self):
        ...
        self.fc_mu.reset_parameters()
        self.fc_std.reset_parameters()
        self.fc_x_a_z.reset_parameters()
        self.q_x_z_mu.reset_parameters()
        self.q_x_z_logvar.reset_parameters()
    ...
    def forward(self, x, edge_index, return_emb = False):
        ...
        std = F.softplus(self.fc_std(x) - self.std_w, beta=ratio("softplus_beta"))
        z = self.reparameterize(mu, std)
        logits = self.convs[-1](z, edge_index)
        x_recon = self.fc_x_a_z(z, edge_index)
        mu_q = self.q_x_z_mu(z)
        log_var_q = self.q_x_z_logvar(z)

        return logits, z, mu, std, x_recon, mu_q, log_var_q

class GCN(nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, num_layers=params("num_layers"),
                 dropout=ratio("dropout"), save_mem=True, use_bn=True, ):
...
class GAT(nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, num_layers=params("num_layers"),
                 dropout=ratio("dropout"), use_bn=False, heads=params("gat_heads"), out_heads=params("out_heads")):
...
class MixHop(nn.Module):
    ...
    """
    def __init__(self, in_channels, hidden_channels, out_channels, num_layers=params("num_layers"),
                 dropout=ratio("dropout"), hops=params("hops")):
...
class GCNJK(nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, num_layers=params("num_layers"),
                 dropout=ratio("dropout"), save_mem=True, jk_type='max'):
...
class GATJK(nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, num_layers=params("num_layers"),
                 dropout=ratio("dropout"), heads=params("gat_heads"), jk_type='max'):
...
class GPR_prop(MessagePassing):
    '''
    GPRGNN, credit goes to https://github.com/jianhao2016/GPRGNN
