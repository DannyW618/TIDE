...
class Logger_detect(object):
    ...
    def print_statistics(self, run=None, return_best_idx=False, dataset=None, choose_epoch = None):
        if run is not None:
            result = 100 * torch.tensor(self.results[run])
...
def save_result(results, args):
    if args.dataset in ('cora', 'amazon-photo', 'coauthor-cs', 'citeseer', 'pubmed'):
        filename = _module_path('results', f'{args.dataset}-{args.ood_type}.csv')
    else:
        filename = _module_path('results', f'{args.dataset}.csv')

    if args.method == 'tide':
        if args.use_prop:
            name = 'tide++' if args.use_reg else 'tide'
        else:
            name = 'tide++ w/o prop' if args.use_reg else 'tide w/o prop'
    else:
        name = args.method
