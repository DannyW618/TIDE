...
from os import path
from param_utils import dataset_aliases, ratio
...
def load_twitch_dataset(data_dir):
    ...
    subgraph_names = dataset_aliases("twitch_regions")
    train_idx, valid_idx = dataset_aliases("twitch_train_eval_slots")
    dataset_ood_te = []
...
def load_arxiv_dataset(data_dir, time_bound=None, inductive=True):
    from ogb.nodeproppred import NodePropPredDataset

    if time_bound is None:
        time_bound = list(dataset_aliases("arxiv_window"))
    ...
    test_year_bound = list(dataset_aliases("arxiv_eval_bounds"))
    ...
    print("dataset, dataset_ind, dataset_ood_tr, dataset_ood_te", dataset, dataset_ind, dataset_ood_tr, dataset_ood_te)
    return dataset_ind, dataset_ood_tr, dataset_ood_te
...
def load_proteins_dataset(data_dir, inductive=True):
    ...
    ind_species_min_idx, ind_species_max_idx, ood_tr_species_max_idx, ood_te_species_stop = dataset_aliases("proteins_species_edges")
    ind_species_min, ind_species_max = species[ind_species_min_idx], species[ind_species_max_idx]
    ood_tr_species_min, ood_tr_species_max = species[ind_species_max_idx], species[ood_tr_species_max_idx]
    ood_te_species = [species[i] for i in range(ood_tr_species_max_idx, ood_te_species_stop)]
...
def create_sbm_dataset(data, p_ii=ratio("sbm_in"), p_ij=ratio("sbm_out")):
    n = data.num_nodes
...
def create_label_noise_dataset(data):
    ...
    y = data.y
    n = data.num_nodes
    idx = torch.randperm(n)[:int(n * ratio("label_noise_fraction"))]
    y_new = y.clone()
    y_new[idx] = torch.randint(0, y.max(), (int(n * ratio("label_noise_fraction")), ))
...
def load_graph_dataset(data_dir, dataname, ood_type):
    ...
    if ood_type == 'structure':
        dataset_ood_tr = create_sbm_dataset(dataset, p_ii=ratio("sbm_in"), p_ij=ratio("sbm_out"))
        dataset_ood_te = create_sbm_dataset(dataset, p_ii=ratio("sbm_in"), p_ij=ratio("sbm_out"))
    elif ood_type == 'feature':
        dataset_ood_tr = create_feat_noise_dataset(dataset)
        dataset_ood_te = create_feat_noise_dataset(dataset)
    elif ood_type == 'label':
        class_t = dataset_aliases("label_cutoffs")[dataname]
        label = dataset.y
